...
def discover_databases(instance_cfg: dict) -> list[dict]:
    ...
    if not _PYODBC_AVAILABLE:
        logger.warning("pyodbc não disponível — SQL Server ignorado neste ambiente")
        return []
...
def collect(cfg: dict) -> dict:
    ...
    if not _PYODBC_AVAILABLE:
        logger.warning("pyodbc não disponível — pulando coleta SQL Server para %s", cfg.get("host"))
        return {"db_name": cfg.get("name",""), "db_type": "sqlserver", "host": cfg.get("host",""),
                "status": "offline", "error": "pyodbc não instalado neste ambiente"}
    ...
    try:
        ...
        for row in cursor.fetchall():
            result["expensive_queries"].append({
                ...
                "last_execution_time": str(row[8]) if row[8] else None,
            })
