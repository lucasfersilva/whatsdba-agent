...
# Remove venv corrompido ou criado como root (dono diferente do SERVICE_USER)
if [[ -d "$VENV_DIR" ]]; then
  VENV_OWNER=$(stat -c '%U' "$VENV_DIR" 2>/dev/null || echo "root")
  if [[ ! -f "$VENV_DIR/bin/pip" || "$VENV_OWNER" != "$SERVICE_USER" ]]; then
    warn "venv inválido ou criado como outro usuário ($VENV_OWNER), removendo..."
    rm -rf "$VENV_DIR"
  fi
fi

if [[ ! -d "$VENV_DIR" ]]; then
  info "Criando venv como usuário $SERVICE_USER..."
  sudo -u "$SERVICE_USER" "$PYTHON" -m venv "$VENV_DIR" \
    || err "Falha ao criar venv. Instale python3-venv: sudo apt install python3.12-venv"
  ok "venv criado"
else
